...
def objective(id):
    ...
    # acc = read_vgg('/home/zss/PycharmProjects/pythonProject/vgg/test.csv')
    runtime = -1.0 * data[0]
    power = -1.0 * data[1]
    obj = [runtime, power]
    # obj = [acc, power]
    # f1 = open("/home/zss/PycharmProjects/pythonProject/vgg/flexibo.csv", "a")
...
async def main(test_config, init_id, os_setting, app_setting):
    ...
    while task_id < test_config.optimizer.iter_limit:
        ...
        # f4 = open("/home/zss/PycharmProjects/pythonProject/lenet/test.csv", "a")
        # print(task_id, file=f4)
        ...
        if task_id != 0:  # not adding default info, 'cause default cannot convert to numeric form
            ...
            f3 = open("/home/zss/PycharmProjects/pythonProject/data/userspace/rnn.csv", "a")
            print(-metric[0], -metric[1], file=f3)
            # f3 = open("/home/zss/PycharmProjects/pythonProject/data/output_para_imp.csv", "a") print(
            # sampled_config['cpu_freq'], sampled_config['dropout_rate'], sampled_config['learn_rate'],
            # sampled_config['batch_size'], sampled_config['inter_op'], sampled_config['intra_op'], sampled_config[
            ...
            # 'tcp_slow_start_after_idle'], sampled_config['dirty_writeback_centisecs'], sampled_config[
            # 'overcommit_ratio'], sampled_config[ 'tcp_tw_reuse'],-1 * metric[0], -1 * metric[1], file=f3)
