...
def objective(id):
    ...
    # obj = [runtime, power]
    obj = [acc, power]
    f1 = open("/home/zss/PycharmProjects/pythonProject/lenet/pabo.csv", "a")
    print(id, -runtime, -power, acc, file=f1)
    return obj
...
async def main(test_config, init_id, os_setting, app_setting):
    ...
    while task_id < test_config.optimizer.iter_limit:
        ...
        if task_id == 0:  # use default config
            ...
            try:
                sampled_config_numeric, sampled_config, flag = optimizer.get_conf()
        ...
        if task_id != 0:  # not adding default info, 'cause default cannot convert to numeric form
            ...
            if flag == 0:
                optimizer.add_observation_to_all(
                    (sampled_config_numeric, metric_result)
                )
            else:
                if flag % 2 == 0:
                    optimizer.add_observation_to_Mp(
                        (sampled_config_numeric, metric_result)
                    )
                else:
                    optimizer.add_observation_to_Mt(
                        (sampled_config_numeric, metric_result)
                    )
            # f3 = open("/home/zss/PycharmProjects/pythonProject/data/output_lenet.csv", "a")
            # print(sampled_config['cpu_freq'], sampled_config['dropout_rate'], sampled_config['learn_rate'],
            #       sampled_config['batch_size'], sampled_config['inter_op'], sampled_config['intra_op'],
            #       sampled_config['dirty_background_ratio'], sampled_config['dirty_expire_centisecs'],
            #       sampled_config['dirty_ratio'], sampled_config['max_map_count'], sampled_config['netdev_max_backlog'],
            #       sampled_config['nr_requests'], sampled_config['read_ahead_kb'], sampled_config['rq_affinity'],
            #       sampled_config['somaxconn'], sampled_config['swappiness'], sampled_config['tcp_abort_on_overflow'],
            #       sampled_config['tcp_max_syn_backlog'], sampled_config['vfs_cache_pressure'],
            #       sampled_config['tcp_slow_start_after_idle'], -1 * metric[0], -1 * metric[1], file=f3)
